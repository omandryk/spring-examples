...
import com.icoderman.config.JdbcConfig;
...
public class JdbcUserRepositoryTest {
  ...
  public void save_existingSpitter() {
    assertEquals(4, userRepository.count());
    User user = new User(4L, "arthur", "letmein", "Arthur Names","arthur@habuma.com");
    User saved = userRepository.save(user);
    assertUser(5, saved);
    assertEquals(4, userRepository.count());
    User updated = userRepository.findOne(4L);
    assertUser(5, updated);
  }
  ...
  private static void assertUser(int expectedUserIndex, User actual, String expectedStatus) {
